...
unsigned short lcd_read_panelid(void)
{
    ...
    /* GPIO初始化 */
    lcdio_config.direction = KGPIO_DigtalOutput;
    ...
    lcdio_config.direction = KGPIO_DigtalInput;
...
void lcdgpio_init(void)
{
    ...
    bl_config.direction = KGPIO_DigtalOutput;
...
void lcd_clear(unsigned int color)
{
	...
	num = (unsigned int)(tftlcd_dev.width * tftlcd_dev.height);					/* 宽度乘以高度等于用到的字节数（像素点）。一个像素点等于4个字节 */
