...
/* imc20608初始化 */
unsigned char imc20608_init(void)
{
    ...
    IOMUXC_SetPinMux(IOMUXC_UART2_CTS_B_ECSPI3_MOSI, 0);                /* 复用ECSPI3_MOSI */
    IOMUXC_SetPinMux(IOMUXC_UART2_RTS_B_ECSPI3_MISO, 0);                /* 复用ECSPI3_MISO */

    IOMUXC_SetPinConfig(IOMUXC_UART2_RX_DATA_ECSPI3_SCLK, 0x10b0);      /* 设置电气属性 SW_PAD */
    ...
    cs_config.direction = KGPIO_DigtalOutput;
...
unsigned char icm20608_read_reg(unsigned char reg)
{
    ...
    ICM20608_CSN(0);                                    /* 片选拉低选中芯片（宏定义） */

    spich0_readwrite_byte(ECSPI3, reg);                 /* 发送要读取的寄存器地址 */
    reg_val = spich0_readwrite_byte(ECSPI3, 0xFF);      /* 0XFF是无效数据，没有意义，只是为了提供一个时钟。从机返回寄存器数据 */
...
{
    reg &= ~0x80;                                        /* 地址的bit7置0，表示写取数据 */
    ICM20608_CSN(0);                                    /* 片选拉低选中芯片（宏定义） */
    spich0_readwrite_byte(ECSPI3, reg);                 /* 写数据的寄存器地址 */
