...
{
    if((base->I2SR & (1 << 5)) && ((base->I2CR) & (1 << 5)) == 0)           /* 如果位1 ，I2C忙 */
    {
        return 1;
    }
    ...
    /* 设置为主机发送模式 */
    base->I2CR |= (1 << 4) | (1 << 2);
    base->I2DR = ((unsigned int)address << 1 ) | ((direction == kI2C_Read)? 1:0);       /* 是否相等，相等返回1，不相等返回0 */
    return I2C_STATUS_OK;
}
